...
const updateScheduledMessageFromStatus = async (socialAccountId: string, status: WhatsAppWebhookStatus) => {
    if (!status.id) {
        return null;
    }
    ...
    if (
        ["sent", "delivered", "read"].includes(status.status || "") &&
        scheduledMessage.status !== WhatsAppScheduledMessageStatus.SENT
    ) {
        return prisma.whatsAppScheduledMessage.update({
            where: {
                id: scheduledMessage.id,
